...
public class MyFileLogListener implements ITestListener {
    ...
    public void onTestFailure(ITestResult iTestResult) {
        Path report = Paths.get("D:\\projects\\AQATests\\reports\\FailedTest_" +
        ...
        takeScreenshot(iTestResult.getMethod().getMethodName() + "_" + LocalDateTime.now().toString().replace(":", "-"));
    }

    @Override
    public void onTestSkipped(ITestResult iTestResult) {
        Path report = Paths.get("D:\\projects\\AQATests\\reports\\SkippedTest_" +
                LocalDateTime.now().toString().replace(":", "-")
                + ".txt");
    ...
    private void takeScreenshot(String fileName) {
        File scrFile = ((TakesScreenshot)MethodsFactory.driver).getScreenshotAs(OutputType.FILE);

        try {
            FileUtils.copyFile(scrFile, new File("D:\\projects\\AQATests\\screens\\"+fileName+".png"));
        } catch (IOException e) {
